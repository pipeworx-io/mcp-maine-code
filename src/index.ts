 ...
 *
 * Title and section, both in the citation: "17-A M.R.S. § 201" becomes
 * /statutes/17-A/title17-Asec201.html. Titles carry letters (17-A), which the
 * path repeats in two different shapes, so the URL is built rather than
 * guessed.
 *
...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Maine Revised Statutes');
}

const BASE = 'https://legislature.maine.gov/statutes';
const UA = 'pipeworx-mcp-maine-code/1.0 (+https://pipeworx.io)';

const tools: McpToolExport['tools'] = [{
  name: 'me_revised_statute',
  description:
  ...
  inputSchema: {
    type: 'object',
    properties: {
      title: { type: 'string', description: 'Maine title, e.g. "17-A" (crimes), "29-A" (motor vehicles), "19-A" (domestic relations).' },
      section: { type: 'string', description: 'Section within the title, e.g. "201" (murder) or "2411" (OUI).' },
    },
...
async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  if (name !== 'me_revised_statute') throw new Error(`Unknown tool: ${name}`);
  const title = String(args.title ?? '').trim().toUpperCase().replace(/\s+/g, '');
  const section = String(args.section ?? '').trim().replace(/\s+/g, '');
  if (!/^\d+(-[A-Z])?$/.test(title) || !/^\d+[-A-Z0-9]*$/.test(section)) {
    return { found: false, reason: 'unparsed_section',
      message: `"${args.title} M.R.S. ${args.section}" is not a citation this can resolve.`,
      hint: 'Maine titles look like "17-A" (crimes) and sections like "201" (murder).' };
  }
  const url = `${BASE}/${title}/title${title}sec${section}.html`;
  const res = await pwFetch(url, { headers: { 'User-Agent': UA } });
  if (res.status === 404) {
    return { found: false, reason: 'section_not_found', title, section,
      message: `No section ${section} in Maine title ${title} (HTTP 404).`,
      hint: 'Section numbers are not contiguous — repealed sections are normal.' };
  }
  if (!res.ok) throw new Error(`Maine Legislature returned HTTP ${res.status}`);
  const html = await readStatuteResponse(res);
  // Take the section-content container: converting the whole page puts the
  // site masthead where the statute should be.
  const text = textFromContainer(html, /class="[^"]*section-content[^"]*"/) ?? statuteText(html);
  return {
    citation: `${title} M.R.S. § ${section}`, title, section,
    text, text_chars: text.length, url,
    source: 'Maine Legislature (legislature.maine.gov), keyless',
    attribution: 'Maine statutes are public record.',
  };
}
